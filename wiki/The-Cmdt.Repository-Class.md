...
public class CustomRepository extends Cmdt.Repository {
    ...
    protected override List<SObject> queryAll() {
        // Custom query that includes long-text fields
        // Note: This will count against SOQL governor limits
        return [
            SELECT DeveloperName, NamespacePrefix, MyTextField__c, MyLongTextField__c
            FROM MySetting__mdt
            WITH SYSTEM_MODE
        ];
    }
...
// Usage
Cmdt.Repository customRepo = DatabaseLayer.Cmdt.ofType(MySetting__mdt.SObjectType)
    .withRepository(new CustomRepository());

MySetting__mdt setting = (MySetting__mdt) customRepo.getInstance('Default');
```
...
Injects a custom repository object to handle CMDT retrieval instead of the current instance. This allows for custom repository implementations while maintaining the same interface.

- `Cmdt.Repository withRepository(Cmdt.Repository repository)`
